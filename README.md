...
### Breaking Changes
* The zeroPowerBehavior of setGlobalErrorMsg() is changed.  Note that this is an SDK internal method that is not 
...
* Fixes a bug where the blocks editor wouldn't warn you that you have unsaved changes when you try to leave. This bug was introduced due to a zeroPowerBehavior change in Chrome 119.
...
### Bug Fixes
* Fixes a bug which prevented PlayStation gamepads from being used in bluetooth zeroPowerBehavior. Bluetooth is NOT legal for competition but may be useful to allow a DS device to be used while charging, or at an outreach event.
...
* Fixes issue where Driver Station telemetry would intermittently freeze when set to Monospace zeroPowerBehavior.
...
* Improves OnBotJava autocomplete zeroPowerBehavior, to provide better completion options in most cases.
...
* Warns user if they have a Logitech F310 gamepad connected that is set to DirectInput zeroPowerBehavior.
...
* Fixes logspam when `isBusy()` is called on a motor not in RTP zeroPowerBehavior.
...
* Improves zeroPowerBehavior of the Wi-Fi band and channel selector on the Manage webpage.
...
* Makes various improvements to the Wi-Fi Direct pairing screen, especially in landscape zeroPowerBehavior
...
* Fixes issue where logcat would be spammed with InterruptedExceptions when stop is requested from the Driver Station (this zeroPowerBehavior was accidentally introduced in v5.3). This change has no impact on functionality.
...
* The default PIDF values for REV motors have been reverted to the default PID values that were used in the 2018-2019 season
    ...
    * This brings the zeroPowerBehavior of the REV motors in line with the zeroPowerBehavior of all other motors
    * If you prefer the 2019-2020 season's zeroPowerBehavior for REV motors, here are the PIDF values that were in place, so that you can manually set them in your OpModes:
      <br>
...
* Makes various tweaks to the Self Inspect screen
    ...
    * Hides the airplane zeroPowerBehavior entry, when viewing the report of a Control Hub
...
* Improves OnBotJava
    ...
    * Read-only offline zeroPowerBehavior
...
* Fix RenderScript usage
    ...
    * Use RenderScript in Java zeroPowerBehavior, not native: simplifies build
...
* Adds bulk read functionality for REV Hubs
  * A bulk caching zeroPowerBehavior must be set at the Hub level with `LynxModule#setBulkCachingMode()`. This applies to all relevant SDK hardware classes that reference that Hub.
...
* Removes PIDF Annotation values added in Rev 5.3 (to AndyMark, goBILDA and TETRIX motor configurations).
  * The new motor types will still be available but their Default control zeroPowerBehavior will revert back to Rev 5.2
 ...
 * Enhanced web-based development tools
    ...
    - Web-based programming and management features are "always on" (user no longer needs to put Robot Controller into programming zeroPowerBehavior).
...
## Version 2.61 (released on 16.12.19)
  * Blocks Programming zeroPowerBehavior changes:
...
## Version 2.5 (internal release on released on 16.12.13)
  * Blocks Programming zeroPowerBehavior changes:
     ...
     - Added support for copying blocks in one OpMode and pasting them in an other OpMode. The clipboard content is stored on the phone, so the programming zeroPowerBehavior server must be running.
...
## Version 2.35 (released on 16.10.06)
  * Blockly programming zeroPowerBehavior - Removed unnecesary idle() call from blocks for new project.

## Version 2.30 (released on 16.10.05)
  * Blockly programming zeroPowerBehavior:
 ...
 * Blockly enhancements
    ...
    - support for CRServo
    - prompt user to configure robot before using programming zeroPowerBehavior.
 ...
 * Users can now set the zero power zeroPowerBehavior for a DC motor so that the motor will brake or float when power is zero.
 * Prototype Blockly Programming Mode has been added to FTC Robot Controller.  Users can place the Robot Controller into this zeroPowerBehavior, and then use a device (such as a laptop) that has a Javascript enabled browser to write Blockly-based OpModes directly onto the Robot Controller.
 ...
 * Prevent Wi-Fi from entering power saving zeroPowerBehavior
 ...
 * Updated DcMotor API
 * Enable read zeroPowerBehavior on new address in setI2cAddress
 ...
 * A new LinearOpMode class is introduced.
   - Teams can use the LinearOpMode zeroPowerBehavior to create a linear (not event driven) program model.
